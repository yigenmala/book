    ...
    $.fn.thumbox = function(settings) {
        ...
        var showImage = function(cord, urlImg, label, imgindex){
            ...
            objImagePreloader.onload  = function(){
                ...
                img.src   = urlImg;
                imgWidth  = objImagePreloader.width;//图片宽度
                imgHeight = objImagePreloader.height;//图片高度
                var h = (375/imgHeight).toFixed(2);//图片宽和外框宽度的比例
                var w = (500/imgWidth).toFixed(2);//图片高和外框高度的比例
                if(h<1||w<1){//长或者宽超出外框范围的话
                    if(h<w){//如果高比宽的                        
                        imgWidth = imgWidth*h;                        
                        imgHeight = imgHeight*h;     

                    }else{
                        imgWidth = imgWidth*w;
                        imgHeight = imgHeight*w;
                    }
